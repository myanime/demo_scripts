...
proxyStartNumberArray = [0, 20, 40, 60, 80, 100]
#choose to select the post to delete(it your flaglist.csv)
addFlagNumberArray = [0, 0, 0, 0, 0, 0]
...
def loadScraper(goodProxyNumber, addFlagNumber, threadNumber):
    try:
        ...
        myuserAgent = user_agents.set_agent()
        #print myuserAgent
        profile.set_preference("general.useragent.override", myuserAgent)

        #profile.set_preference("general.useragent.override",'Mozilla/5.0 (X11; Linux i686) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/45.0.2454.101 Safari/537.36')
        ...
        for addToBeFlagged in itertools.islice(myAddToBeFlaggedList, startAddFlag, endAddFlag):
            ...
            try:
                ...
                try:
                    ...
                    #browser.get('http://www.myipaddress.com/show-my-ip-address/')
                    time.sleep(2)
...
def moveOnToNextProxy(browser, goodProxyNumber, addFlagNumber, threadNumber):
    browser.quit()
    myNumber = random.randrange(5, 30, 1)
    time.sleep(myNumber)
